...
describe("User Model Test Suite", () => {
  ...
  it("should not allow duplicate emails", async () => {
    ...
    expect(err).toBeDefined();
    expect(err.code).toBe(11000); // Duplicate email
  });

  // Inside the relevant tests:

it("should hash password before saving", async () => {
  const user = new User({
    username: "hashuser",
    email: "hashuser@example.com",
    password: "SecretPass1!",   // Added special char and digit
  });
...
it("should compare password correctly", async () => {
  const user = new User({
    ...
    password: "MatchMe123!",  // Added special char
  });
  await user.save();
...
it("should allow adding to savedSnippets", async () => {
  const user = new User({
    ...
    password: "SnippetPass1!",  // Added special char and digit
  });
